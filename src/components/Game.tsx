...
import type { WordPair, GameSession, PlayerWithRole } from '../types/index';
...
export default function Game() {
  ...
  function addPlayer() {
    ...
    const word = role === 'undercover' ? chosenPair.undercover : role === 'civilian' ? chosenPair.civilian : null;
  ...
  function startGame() {
    ...
    for (let i = 0; i < shuffledIndices.length && undercoverCount < requiredRoles.undercover; i++) {
    ...
    let mrwhiteCount = 0;
    const remainingIndices = shuffledIndices.filter((i) => assigned[i].role === 'civilian');

    for (let i = 0; i < remainingIndices.length && mrwhiteCount < requiredRoles.mrwhite; i++) {
  ...
  function checkGameEnd(
    ...
    if ((undercoverAlive || (undercoverAlive && mrwhiteAlive)) && !civilianAlive) return 'undercover';
  ...
  function reveal(index: number) {
    ...
    const playersUpdated = session.players.map((p, i) => (i === index ? { ...p, revealed: true } : p));
  ...
  function submitMrWhiteGuess() {
    ...
    const playersUpdated = session.players.map((p) => (p.id === guessModal.player!.id ? { ...p, revealed: true } : p));
    ...
    <div className="">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80">
          <div className="bg-stone-900 rounded-lg p-8 w-full max-w-md mx-4 text-center text-white">
            ...
            <p className="text-lg mb-8 text-white/80">
              Choose how many players will be in this session to ensure proper role distribution.
            </p>
            ...
            <div className="text-sm text-white/60">
              Role distribution is automatically calculated to ensure a balanced game.
            </div>
          </div>
        </div>
      )}
      ...
      {!isGameStarted ? (
        <div className="bg-white/10 backdrop-blur-sm rounded-lg p-6">
          ...
          <div className="flex gap-2 mb-4">
            ...
            <button
              onClick={addPlayer}
              disabled={!welcomeModal.playerCount || players.length >= (welcomeModal.playerCount || 0)}
          ...
          {welcomeModal.playerCount && players.length >= welcomeModal.playerCount && (
            <div className="mb-4 p-3 bg-yellow-900/20 rounded-lg border border-yellow-500/30 text-yellow-200 text-sm">
              Maximum players reached ({welcomeModal.playerCount}). You can now start the game or change the player
              count.
            </div>
          )}
          ...
          {welcomeModal.playerCount && (
            <div className="mb-4 p-4 bg-blue-900/20 rounded-lg border border-blue-500/30">
              <h3 className="font-medium text-blue-300 mb-2">Session Configuration</h3>
              <div className="text-sm text-white/80">
                <div>
                  Total Players: <span className="font-semibold">{welcomeModal.playerCount}</span>
                </div>
                <div className="mt-1">
                  {(() => {
                    const roles = calculateRequiredRoles(welcomeModal.playerCount);
                    return (
                      <>
                        <span className="text-red-400">{roles.undercover} Undercover</span> •
                        <span className="text-yellow-400"> {roles.mrwhite} Mr. White</span> •
                        <span className="text-green-400"> {roles.civilian} Civilians</span>
                      </>
                    );
                  })()}
                </div>
                <button
                  onClick={() => setWelcomeModal({ open: true, playerCount: null })}
        ...
        <div className="bg-white/10 backdrop-blur-sm rounded-lg p-6">
            ...
            {session?.players.map((p, i) => (
              <div
                ...
                className={`bg-white/10 rounded-lg p-4 text-center cursor-pointer transition-all hover:scale-105 ${
                  p.revealed ? 'ring-2 ring-green-400 bg-green-900/20' : 'hover:bg-white/20'
                }`}
              ...
              >
                  ...
                  <div className="space-y-2">
                    <div
                      className={`text-sm px-2 py-1 rounded-full ${
                        p.role === 'undercover' ? 'bg-red-600' : p.role === 'mrwhite' ? 'bg-yellow-600' : 'bg-green-600'
                      }`}
                    >
                      {p.role === 'undercover' ? 'Undercover' : p.role === 'mrwhite' ? 'Mr. White' : 'Civilian'}
                    </div>
                ...
                {!p.revealed && <div className="text-white/60">Click to reveal</div>}
              </div>
            ))}
          </div>
          ...
          {session?.winner === 'mrwhite' ? (
            <div className="mt-6 text-center">
              <div className="text-2xl font-bold mb-2">The Mr. White player won!</div>
            </div>
      ...
      {revealModal.open && revealModal.player && (
        <div className="fixed m-0 inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-stone-900 rounded-lg p-6 w-full max-w-md mx-4 text-center text-white">
            <h3 className="text-xl font-semibold mb-2">Hello, {revealModal.player.name}!</h3>
            ...
            {revealModal.player.role === 'mrwhite' ? (
              ...
              <div className="text-lg font-bold mb-4">Your word: `&quot;`{revealModal.player.word}`&quot;`</div>
            )}
      ...
      {guessModal.open && guessModal.player && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          <div className="absolute inset-0 bg-black/60" onClick={closeGuessModal} />
          <div
            className="relative
            bg-stone-900
            rounded-lg
          ...
          >
            ...
            <input
              ...
              onChange={(e) => setGuessModal((g) => ({ ...g, guess: e.target.value }))}
            ...
            {guessModal.feedback && <div className="text-sm text-yellow-300 mb-2">{guessModal.feedback}</div>}
